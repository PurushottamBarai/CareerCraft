...
function setupEventListeners() {
    ...
    document.getElementById('employerRegisterForm').addEventListener('submit', handleEmployerRegister);
...
function updateNavigation() {
    ...
    if (currentUser) {
        ...
        if (welcomeMessageEl) welcomeMessageEl.textContent = `Welcome, ${currentUser.firstName}!`;
...
async function handlePostJob(e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const msgElement = document.querySelector('#postJobSection .success-message');
            if (msgElement) {
...
async function loadEmployerJobs() {
    ...
    try {
        ...
        if (jobs.length === 0) {
            jobsList.innerHTML = '<div style="padding: 2rem; text-align: center; color: #666;">No jobs posted yet.</div>';
            return;
        }
        ...
        jobs.forEach(job => {
            ...
            jobRow.innerHTML = `
                ...
                <div>${Array.isArray(job.skills) ? job.skills.join(', ') : 'No skills listed'}</div>
                <div>${job.experienceYears || 0}y ${job.experienceMonths || 0}m</div>
                <div>${job.applicationCount || 0}</div>
                <div>
                    <button class="view-btn" onclick="viewApplications(${job.id})">
...
async function loadAvailableJobs() {
    ...
    try {
        ...
        if (jobs.length === 0) {
            jobsList.innerHTML = '<div style="padding: 1rem; text-align: center; color: #666;">No jobs available at the moment.</div>';
            return;
        }
        ...
        jobs.forEach(job => {
            ...
            const jobRow = document.createElement('div');
            jobRow.className = 'job-row';
            const companyName = job.companyName || `${job.employerFirstName} ${job.employerLastName}`;
            jobRow.innerHTML = `
                <div>${job.title}</div>
                <div>${companyName}</div>
                ...
                <div>
                    <button class="apply-btn" onclick="openApplyModal(${job.id}, '${job.title}', '${companyName}')">
                        Apply
                    </button>
                </div>
            `;
    ...
    try {
        ...
        applications.forEach(app => {
            ...
            appRow.innerHTML = `
                ...
                <div>${app.companyName || app.employerFirstName + ' ' + app.employerLastName}</div>
    ...
    try {
        ...
        statsContainer.innerHTML = `
            ...
            <div class="stat-card success">
                <span class="stat-number">${stats.acceptedApplications || 0}</span>
    ...
    try {
        ...
        statsContainer.innerHTML = `
            ...
            <div class="stat-card danger">
                <span class="stat-number">${stats.rejectedApplications || 0}</span>
    ...
    try {
        ...
        const applicationsHTML = applications.map(app => `
            <div style="border: 1px solid #ddd; padding: 1rem; margin: 0.5rem 0; border-radius: 5px;">
                <strong>${app.studentFirstName} ${app.studentLastName}</strong><br>
                ...
                Status: <span class="status-${app.status}">${app.status.toUpperCase()}</span><br>
                ${app.resumePath ? `<a href="${app.resumePath}" target="_blank">View Resume</a><br>` : ''}
    ...
    try {
        ...
        profileDetails.innerHTML = `
            <div style="display: grid; gap: 1rem;">
                ...
                ${profile.companyName ? `<div><strong>Company:</strong> ${profile.companyName}</div>` : ''}
                ${profile.college ? `<div><strong>College:</strong> ${profile.college}</div>` : ''}
                ${profile.course ? `<div><strong>Course:</strong> ${profile.course}</div>` : ''}
                ${profile.graduationYear ? `<div><strong>Graduation Year:</strong> ${profile.graduationYear}</div>` : ''}
...
function loadEditProfile() {
    ...
    if (currentUser.role === 'employer') {
        ...
        if (form.companyName) form.companyName.value = currentUser.companyName || '';
    } else {
        employerFields.classList.add('hidden');
        studentFields.classList.remove('hidden');
        if (form.college) form.college.value = currentUser.college || '';
        if (form.course) form.course.value = currentUser.course || '';
        if (form.graduationYear) form.graduationYear.value = currentUser.graduationYear || '';
    }
}
